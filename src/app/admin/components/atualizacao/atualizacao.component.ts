...
export class AtualizacaoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // lancamentoId é passado pela URL (path: 'atualizacao/:lancamentoId')
  ...
  obterDadosLancamento() {
    this.lancamentoService.buscarPorId(this.lancamentoId) //acabou de adquirir em 'this.route.snapshot.paramMap.get('lancamentoId')'
      .subscribe(
        dados => {
          const data = dados.data.data; // obtendo a data em ingles e convertendo para  o formato que definirmos
          this.form.get('data').setValue(data.substr(0, 10)); // dd/mm/aaaa
          // obtenha o elemento data(o mesmo para os demais campops abaixo) do formulario e atribua a ele esses novos valores 
          this.form.get('horas').setValue(data.substr(11, 2)); // horas
          this.form.get('minutos').setValue(data.substr(14, 2)); // minutos
          this.form.get('tipo').setValue(dados.data.tipo);
          this.localizacao = dados.data.localizacao; // obtendo a localização porque a api exige essa informação
        },
        err => {
